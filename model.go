...
func (p *product) halfPriceAllProducts(db *sql.DB) error {
	_, err :=
		db.Exec("UPDATE products SET price = price/2")
...
func getCheapProducts(db *sql.DB, maxPrice int) ([]product, error) {
	rows, err := db.Query(
